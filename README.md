...
Requirements
Node.js v16 or later (uses native ES modules and fs.watch)
No external dependencies — nothing to install via npm install
Setup
Clone the repo:
git clone https://github.com/Guilherme-N-Melo/autoscripTS.git
Place it alongside the project you want to manage, and adjust the watched folder and target package.json paths in runGen.js if your layout differs from the example above.
Usage
...
This is different from where the autoscripTS folder must sit: it still needs to stay a sibling of src/ and the target package.json (see Project structure), since the code looks for them via ../src and ../package.json relative to its own location on disk.
